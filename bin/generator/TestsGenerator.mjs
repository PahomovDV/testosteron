...
class TestsGenerator extends Base {
    constructor({ model, config, logger }) {
        ...
        this.folders = {
            ...
            siteApiTestSuites  : path.join(config.testsFolder, 'api/site-api', this.config.namespace),
        ...
        this.files = {
            ...
            siteApiTester       : path.join(this.folders.testsLib, 'SiteAPITester.mjs'),
    ...
    async generate(options, layers) {
        const foldersArray = Object.keys(this.folders).map(key => [ this.folders[key] ]);

        for (const folder of foldersArray) {
            if (!this.fileExists(folder[0]) && folder[0] !== '') {
                await fs.mkdir(folder[0], { recursive: true }, (err) => {
                    if (err) {
                        throw err;
                    }

                    this.logger.header(`Dir created : ${folder[0]}`);
                });
            }
        }
        ...
        // if (layers['test-site-api']) {
        //     this.logger.header('Generate Test Suites: SiteAPI');
        //     await this.#generateSiteApiTestSuites(options);
        // }
    ...
    #generateSiteApiTestSuites = async ({ override, actor, actions } = {}) => {
        if (!actor) throw new Error('"actor" is required to generate tests');
        const actorNameVariants = this.generateModelNameVariants(actor);
        const useCases = actions.split('').map(action => this.actionsMap[action]);
        ...
        for (const useCase of useCases) {
            const useCaseName = `${this.modelNames.plural_lcc}-${useCase}`;
            const useCaseFixturesPath = path.join(this.folders.useCaseFixtures, useCaseName);
            const useCaseTestSuitePath = path.join(this.folders.siteApiTestSuites, `${useCaseName}.test.mjs`);
            const testSuiteTemplatePath = path.join(this.folders.templates, 'site-api', useCase);
            ...
            const suiteString = this.fillTemplateWithData(testSuiteTemplate, {
                ...
                'MODEL_NAME_toLC'        : this.modelNames.lcc,
                'ACTOR_toLC'             : actorNameVariants.lcc,
                'ROOT_RELATIVE_PATH'     : path.relative(this.folders.siteApiTestSuites, process.cwd()),
                'TESTER_RELATIVE_PATH'   : path.relative(this.folders.siteApiTestSuites, this.files.siteApiTester),
                'FIXTURES_RELATIVE_PATH' : path.relative(this.folders.siteApiTestSuites, useCaseFixturesPath)
            });
