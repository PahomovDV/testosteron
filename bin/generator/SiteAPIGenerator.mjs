...
class SiteAPIGenerator extends Base {
    constructor({ model, config, logger }) {
        super({ model, config, logger });

        this.folders = {
            templates   : path.join(config.templatesFolder, 'site-api'),
            controllers : path.join(config.siteApiFolder, config.namespace, 'controllers'),
            usecases    : path.join(config.usecasesFolder, config.namespace, this.modelNames.plural_lcc),
            api         : path.join(config.siteApiFolder, config.namespace),
            apiBase     : config.siteApiFolder
        };
...
export default SiteAPIGenerator;
